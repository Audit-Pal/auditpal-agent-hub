import { useCallback, useDeferredValue, useEffect, useMemo, useState, Suspense, lazy } from 'react'
...
export default function App() {
  ...
  const featuredPrograms = useMemo(() => programs.filter((program) => program.isNew), [programs])
  const kindOptions = useMemo(() => [...new Set(programs.map((program) => program.kind))], [programs])
  const categoryOptions = useMemo(() => [...new Set(programs.flatMap((program) => program.categories || []))], [programs])
  const platformOptions = useMemo(() => [...new Set(programs.flatMap((program) => program.platforms || []))], [programs])
  ...
  }, [deferredSearchQuery, programs, selectedCategory, selectedKind, selectedPlatform, sortBy])
  ...
  const totalResearchersTouching = useMemo(
    () => programs.reduce((total, program) => total + program.scopeReviews, 0).toLocaleString(),
    [programs],
  )

  const liveSignals = useMemo<LiveSignal[]>(() => {
    return programs
      .flatMap((program) =>
  ...
  const sortedReports = useMemo(
    () => [...reports].sort((left, right) => right.submittedAt.localeCompare(left.submittedAt)),
    [reports],
  )
